...
| **for_claude_code marker pinning** | ~~Empirical Claude Code idle prompt regex~~ **Resolved in Phase 3.5** (2026-04-12). Visual pattern matching was the wrong primitive — too fragile to false-positive on punctuation, code blocks, and quoted text. Switched to a **hook-based detection path** using Claude Code's built-in Stop hook system. Per-session `.claude/settings.json` is written to a temp dir (symlinked from `~/.claude` to preserve auth), the Stop hook writes JSON to a FIFO, and a sidecar reader thread in ccom completes active turns using `last_assistant_message` from hook stdin. Regex `for_claude_code()` placeholder remains in the tree as the fallback path for `SessionKind::Terminal` and future non-Claude runners. See `docs/designs/response-boundary-detection.md` and `docs/plans/phase-3.5-hook-boundary.md`. |
